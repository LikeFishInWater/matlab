...
for ii=1:length(EsNo)
    ...
    while flag
        ...
        for p1=1:LdpcLen/2
            ...
            ht(:,p1) = 1/sqrt(pi/2)*(randn(length(K),1) + 1i * randn(length(K),1));
            % yt(:,p1)=x+noise;
            yt(:,p1)=awgn(ht(:,p1).*x,snr);
        end   
            ...
            for p1=1:LdpcLen/2
                ...
                for t=1:T
                    ...
                    for k=1:4
                        for m=1:4
                            for n=1:16
                                p(n)=D(k,h(k,2),a(1,n))*D(k,h(k,3),a(2,n))*exp(-1/(2*sigma^2)*(abs(y(k)./ht(:,p1)-S(k,h(k,1),m)-S(k,h(k,2),a(1,n))-S(k,h(k,3),a(2,n))))^2);
                            end                         
                            Dtemp(k,h(k,1),m)=sum(p);
                        end
                        for m=1:4
                            for n=1:16
                                p(n)=D(k,h(k,1),a(1,n))*D(k,h(k,3),a(2,n))*exp(-1/(2*sigma^2)*(abs(y(k)./ht(:,p1)-S(k,h(k,2),m)-S(k,h(k,1),a(1,n))-S(k,h(k,3),a(2,n))))^2);  
                            end
                            Dtemp(k,h(k,2),m)=sum(p);
                        end
                        for m=1:4
                            for n=1:16
                                p(n)=D(k,h(k,1),a(1,n))*D(k,h(k,2),a(2,n))*exp(-1/(2*sigma^2)*(abs(y(k)./ht(:,p1)-S(k,h(k,3),m)-S(k,h(k,1),a(1,n))-S(k,h(k,2),a(2,n))))^2);
                            end
