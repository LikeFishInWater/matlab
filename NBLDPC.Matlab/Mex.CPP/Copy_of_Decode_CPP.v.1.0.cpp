        ...
        unsigned int **TABLE_MULTIPLY, unsigned int **TABLE_ADD, unsigned int *TABLE_INVERSE)
{
    ...
    else
    {
        ...
        for (unsigned int l = 0; l < qAry - 1; l++)
        {
			alpha_i = l + 1;
            unsigned int first = TABLE_MULTIPLY[TABLE_INVERSE[A1]][alpha_i] - 1;
            unsigned int second = TABLE_MULTIPLY[TABLE_INVERSE[A2]][alpha_i] - 1;
            a = exp(double(L1[first]));
            b = exp(double(L2[second]));
            c = 0;
			unsigned int x;
            for (unsigned int xx = 1; xx < qAry; xx++){
				x = xx - 1;
                if (xx != TABLE_MULTIPLY[alpha_i][TABLE_INVERSE[A1]]){
                    c = c + exp (double(L1[x]) + double(L2[TABLE_MULTIPLY[TABLE_INVERSE[A2]][TABLE_ADD[alpha_i][TABLE_MULTIPLY[xx][A1]]] - 1]));
                }
            }
            d = 0;
            for (unsigned int xx = 1; xx < qAry; xx++){
				x = xx - 1;
                d = d + exp (double(L1[x] )+double( L2[TABLE_MULTIPLY[TABLE_INVERSE[A2]][TABLE_MULTIPLY[xx][A1]] - 1]));
            }
            L_temp[l]= log(a + b + c) - log(1 + d);
        }
		for (unsigned int l = 0; l < qAry - 1; l++)
        {
			L[l] = L_temp[l];
